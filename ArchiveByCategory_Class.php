...
class Posts_By_Category extends WP_Widget {
	...
	function __construct() {
		...
		$widget_args = array('classname' => 'class_archive_by_category', 'description' => 'Permet de lister des archives d\'articles correspondantes à la catégorie que nous sommes en train de visiter.');
		
		//Réglages du formulaire d'administration, permet d'élargir la largeur du bloc widget dans l'admin.
		
		$control_args = array('width' => 350);
...
?>
				<a href="<?php
				echo $article->guid; ?>"><?php
				echo $article->post_title; ?></a>
			</li>
			<?php
			endforeach;
			wp_reset_postdata();
...
?>
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				$('#<?php
		echo $this->get_field_id('font_color'); ?>').wpColorPicker();
			});
		</script>
		<p>
			<label for="<?php
		...
		<p>	
			<label for="<?php
		echo $this->get_field_id('font_color'); ?>" style="display:innline-block; vertical-align:middle;"><?php
		_e('Couleur du titre :'); ?></label> 
			<input style="vertical-align:middle; display:inline-block;" class="color-field" id="<?php
		echo $this->get_field_id('font_color'); ?>" name="<?php
		echo $this->get_field_name('font_color'); ?>" type="text" value="<?php
		echo esc_attr($this->font_color); ?>" />
		</p>	
		<p>	
			<label for="<?php
		echo $this->get_field_id('nb_posts'); ?>"><?php
		_e('Nombre d\'articles à afficher:'); ?></label> 
			<input size="3" id="<?php
		echo $this->get_field_id('nb_posts'); ?>" name="<?php
		echo $this->get_field_name('nb_posts'); ?>" type="text" value="<?php
		echo esc_attr($nb_posts); ?>">	
		</p>
		<p>	
			<label for="<?php
		echo $this->get_field_id('border_radius'); ?>"><?php
		_e('Arrondi des images (en %) :'); ?></label> 
			<input size="3" id="<?php
		echo $this->get_field_id('border_radius'); ?>" name="<?php
		echo $this->get_field_name('border_radius'); ?>" type="text" value="<?php
		echo esc_attr($border_radius); ?>">	
		</p>
		<p>	
			<label for="<?php
		echo $this->get_field_id('see_thumbnail'); ?>"><?php
		_e('Afficher les images à la une :'); ?></label> 
			<input <?php
		checked($instance['see_thumbnail'], 'on'); ?> id="<?php
		echo $this->get_field_id('see_thumbnail'); ?>" name="<?php
		echo $this->get_field_name('see_thumbnail'); ?>" type="checkbox">	
		</p>
		<p>	
			<label for="<?php
		echo $this->get_field_id('see_home'); ?>"><?php
		_e('Affichage sur la page d\'accueil :'); ?></label> 
			<input <?php
		checked($instance['see_home'], 'on'); ?> id="<?php
		echo $this->get_field_id('see_home'); ?>" name="<?php
		echo $this->get_field_name('see_home'); ?>" type="checkbox">	
		</p>
